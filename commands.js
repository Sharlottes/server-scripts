...
amountck=(u,t,p,ii,i)=>ii+i<=Vars.state.rules.unitCap||p.sendMessage((i-1)+"개 소환됨.\n[red]유닛이 너무 많습니다![]\n현재 제한 수: [orange]"+Vars.state.rules.unitCap);
spawnrunner=(a,p)=>{if(!cmdvalid(p))return;u=getu(a,p);t=gett(a,p,2);if(u!=null&&u!=UnitTypes.block){ii=Groups.unit.count(g=>g.team==t&&g.type==u);for(i=0;i<(a[1]||1);i++){if(amountck(u,t,p,ii,i)){spawn(a,p,u,t,i)}else{break}}}};
