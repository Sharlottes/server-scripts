...
spawn=(a,p,u,t)=>u.spawn(t,(a[3]||p.x||8),(a[4]||p.y||8));
amountck=(u,t,p,i)=>Groups.unit.count(g=>g.team==t&&g.type==u)+1<=Vars.state.rules.unitCap||p.sendMessage(i+"개 소환됨.\n[red]유닛이 너무 많습니다![]\n현재 제한 수: [orange]"+Vars.state.rules.unitCap);
spawnrunner=(a,p)=>{if(!cmdvalid(p))return;u=getu(a,p);t=gett(a,p,2);if(u!=null&&u!=UnitTypes.block){for(i=0;i<(a[1]||1);i++){if(amountck(u,t,p,i)){spawn(a,p,u,t)}else{break}}}};
